package dev.gaphunter.jwtcompanion.review
 ...
 * number of successful token decodes -- never on install, never on a
 * timer, and never for a malformed-token result. Counts invocations
 * directly instead of tracking a set of seen keys.
 *
...
object ReviewPrompt {
    ...
    private const val KEY_HIT_COUNT = "dev.gaphunter.jwtcompanion.review.hitCount"
    private const val KEY_ANSWERED = "dev.gaphunter.jwtcompanion.review.answered"

    private const val NOTIFICATION_GROUP_ID = "JWT Companion"

    // Marketplace listing (real ID resolved after first publish).
    private const val MARKETPLACE_URL = "https://plugins.jetbrains.com/plugin/33316-jwt-companion/reviews"

    /**
     * Call this once per real, successful token decode. Safe to call
     * on any thread.
     *
     * A no-op if [ApplicationManager.getApplication] is null -- this
     * panel's own unit tests construct it directly outside the
     * platform (no `BasePlatformTestCase`, see `JwtDecoderPanelTest`),
     * so `PropertiesComponent`/`NotificationGroupManager` are simply
     * unavailable there; there's no real IDE session to show a
     * notification in anyway.
     */
